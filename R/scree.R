...
scree_plot = function(pc_dataframe, pc_number=NULL, cumulative = T) {
  y = ifelse(cumulative == T, cum_variance_proportion(pc_dataframe, pc_number=NULL), variance_proportion(pc_dataframe, pc_number=NULL))
  x = ifelse(cumulative == T, 0:length(y), 1:length(y))
  p = ggplot() +
    aes(x=x,y=y,group=1) +
